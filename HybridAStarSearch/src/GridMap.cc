...
void GridMap::AddPolygonObstacles(geometry_msgs::Polygon p,
  ...
  // the segments are the same size as the points.  这里的障碍物采用的就是栅格法
  ...
  for (int i = 0; i < size; i++) 
  {
    ...
    // DDA 将实际障碍物坐标转换为栅格坐标  // +1 is important!! MUST make sure the segments are enclosed.
    int length =static_cast<int>(std::max(std::abs(start_x - end_x), std::abs(start_y - end_y)) /xy_grid_resolution_) +1;  
...
// heuristic map & obstacle map 产生目标距离映射 以目标点为基础，计算范围内各点的代价
//从终点开始扩展,
bool GridMap::GenerateDestinationDistanceMap() {
  ...
  while (pq_.size() > 0) 
  {
    ...
    std::vector<std::shared_ptr<Node2d>> next_nodes = std::move(GenerateNextNodes(cur_node));
  ...
  std::cout << "Heuristic Map generated successfully! visited size: " << visited.size() << " map size: " << map_2d_.size() << std::endl;

  return true;
}

// 以原始的障碍物边界为基准，计算所有栅格点的障碍物代价 ObstacleDistance越小越危险,0即为障碍物边
bool GridMap::GenerateObstacleDistanceMap() {
  ...
  // 设置每个栅格点的障碍物距离,本身是障碍物和障碍物内部的设置为0,其他根据距离最近的障碍物距离设置.
  ...
  while (!pq.empty()) {
    ...
    for (int i = 0; i < 4; i++) {
      ...
      if (next_node->GetObstacleDistance() > cur_node->GetObstacleDistance() + 1) {
  ...
  for (auto name : border_unavailable_) {
    dq.emplace_front(map_2d_[name]); //在容器开头添加元素
  }
  while (!dq.empty()) {
  ...
  std::cout << "Obstacle Map generated successfully! visited size: " << visited.size() << std::endl;
}
...
// for expansion 
// 扩展节点的过程中，设置了该节点到达目标点的destinationcost 产生4个扩展节点，产生的方向如何定义的？
